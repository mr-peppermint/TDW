...
function VaultShell({
  ...
  return (
    <div className={`vault${scrollable ? ' vault--scrollable' : ''}`}>
    ...
    <nav className="vault__nav" aria-label="Site navigation">
      <div className="vault__nav-brand">
        <a href="/" className="vault__nav-brand-link">
          <span className="vault__nav-theta">&Theta;</span>
      ...
      <div className="vault__login-wrap">
        <div className={`vault__login${shaking ? ' vault__login--shake' : ''}`}>
          ...
          <div className="vault__login-field">
            ...
            />
            {error && (
              <p className="vault__login-error" role="alert">
                {error}
              </p>
            )}
          </div>
          ...
          <button className="vault__login-btn" onClick={attempt} type="button">
            ...
            <span className="vault__login-btn-arrow" aria-hidden="true">&rarr;</span>
    ...
    <VaultShell scrollable>
      ...
      <main className="vault__main" aria-label="Vault file manager">
        ...
        <div
          ...
          onKeyDown={event => {
            if (event.key === 'Enter' && !uploading) fileInputRef.current?.click();
          }}
        >
          <input
            ref={fileInputRef}
          ...
          {uploading ? (
            ...
            <>
              ...
              <p className="vault__dropzone-hint">Max 10 GB per file &middot; executables blocked</p>
            </>
          )}
          ...
          {!loading && filteredFiles.map(file => (
            <div key={file.name} className="vault__file-row">
              ...
              <div className="vault__file-meta">
                <span className="vault__file-name" title={file.name}>
                  {file.name}
                </span>
                <span className="vault__file-info">
                  {file.sizeHuman} &middot; {relativeTime(file.modified)}
                </span>
              </div>
        ...
        <footer className="vault__footer">
          ...
          <span className="vault__footer-sep">&middot;</span>
          <span>Zero-trust backend &middot; path-jailed &middot; rate-limited</span>
        </footer>
      </main>

      <div className="vault__toasts" aria-live="polite" aria-atomic="false">
        {toasts.map(toast => (
          <div key={toast.id} className={`vault__toast vault__toast--${toast.kind}`}>
            {toast.message}
          </div>
        ))}
      </div>
